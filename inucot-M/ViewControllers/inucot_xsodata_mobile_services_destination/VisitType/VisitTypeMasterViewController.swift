...
class VisitTypeMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var v2: V2<OnlineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([VisitType]?, Error?) -> Void) -> Void)?
    private var entities: [VisitType] = [VisitType]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let visitType = self.entities[indexPath.row]

        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "id", value: "\(visitType.id!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! VisitTypeDetailViewController
...
extension VisitTypeMasterViewController: Inucot_xsodata_mobile_services_destinationEntitySetUpdaterDelegate {
