...
class LocationTypeMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var v2: V2<OnlineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([LocationType]?, Error?) -> Void) -> Void)?
    private var entities: [LocationType] = [LocationType]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let locationType = self.entities[indexPath.row]

        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "id", value: "\(locationType.id!)")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! LocationTypeDetailViewController
...
extension LocationTypeMasterViewController: Inucot_xsodata_mobile_services_destinationEntitySetUpdaterDelegate {
