...
class VisitorTypeMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var v2: V2<OnlineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([VisitorType]?, Error?) -> Void) -> Void)?
    private var entities: [VisitorType] = [VisitorType]()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let visitorType = self.entities[indexPath.row]

        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "id", value: "\(visitorType.id!)")
        return cell
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        self.v2.deleteEntity(currentEntity) { error in
            if let error = error {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! VisitorTypeDetailViewController
...
extension VisitorTypeMasterViewController: Inucot_xsodata_mobile_services_destinationEntitySetUpdaterDelegate {
